...
function _chyHats(d){
    ...
    if(caught.hat){
        p.audience = 1;
        p.caughtHat = true;
        d.chyron = "los sombreros ya no molan";
        return true;
    }
    return false;
}
    ...
    if(d.scene.camera.isOverTV(true)){
        ...
        if(caught.crickets.length>0){
            ...
            if(caught.crickets.length==1){
                ...
                d.chyron = "ya vale de bichos";
            }
        }else if(caught.peeps.length>0){
            if(caught.peeps.length==1){
                d.chyron = "alguien cualquiera";
            }else{
                d.chyron = "gente como cualquier otra";
            }
        }else{
            p.ITS_NOTHING = true;
            d.chyron = "IMPRESIONANTE, NO PASA NADA";
        }
    }
    return true;
}
